...
Связь **бизнес-требований (ФТ)**, **системных прецедентов (UC)**, **API** и **сущностей** доменной модели.

Источник бизнес-требований: [техническое задание](../07-management/technical-specification.md).  
Источник системных прецедентов: [спецификация прецедентов](use-case-specification.md).

## Бизнес-требования → Системные прецеденты

| Бизнес-требование | Бизнес-прецедент | Системный UC | API-эндпоинт | Сущность | Статус |
|-------------------|-----------------|--------------|-------------|---------|--------|
| ФТ-01 | Регистрация | UC-01 | POST /auth/signup | User | ✅ |
| ФТ-02 | Вход | UC-02 | POST /auth/login | User | ✅ |
| ФТ-03 | OAuth2 | UC-03 | /oauth2/** | User | ✅ |
| ФТ-04 | Сброс пароля | UC-04, UC-05 | POST /api/users/forgot-password, reset-password | User | ✅ |
| ФТ-13 | Профиль | UC-06…UC-08 | GET/PUT /api/users/profile, DELETE /api/users/delete | User | ✅ |
| ФТ-06, ФТ-07 | Группа | UC-09…UC-14 | /api/groups/** | Group, User | ✅ |
| ФТ-08 | Предметы | UC-15…UC-19 | /api/training-subjects/** | TrainingSubject | ✅ |
| ФТ-09, ФТ-10 | Материалы | UC-20…UC-23 | /api/educational-materials/** | EducationalMaterial | ✅ |
| ФТ-11, ФТ-12 | Выполнение | UC-24, UC-25 | /api/material-complete/{materialId} | MaterialComplete | ✅ |

## Бизнес-прецеденты → Системные прецеденты (детализация)
